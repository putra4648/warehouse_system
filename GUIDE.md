...
- In Clients menu, choose client name `wms-client`, make sure this config is enabled in Capability config
  ...
  - Authentication flow:
    ...
    - Direct access grant is **enabled**
- In Users menu, choose `user1`, if you want to change password in `Credentials` and choose `Reset Password`. Default password `user123`. In Details section remove `update password` in field `Required user actions`, check Email Verified is **enabled** and then Save
